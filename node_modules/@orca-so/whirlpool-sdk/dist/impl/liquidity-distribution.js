...
function getLiquidityDistribution(ctx, poolAddress, tickLower, tickUpper, refresh) {
    var _a, _b;
    return __awaiter(this, void 0, void 0, function* () {
        const datapoints = [];
        const pool = yield ctx.accountFetcher.getPool(poolAddress, refresh);
        if (!pool) {
            throw new Error(`Whirlpool not found: ${(0, anchor_1.translateAddress)(poolAddress).toBase58()}`);
        }
        const tokenDecimalsA = (_a = (yield ctx.accountFetcher.getMintInfo(pool.tokenMintA, false))) === null || _a === void 0 ? void 0 : _a.decimals;
        if (!tokenDecimalsA) {
            throw new Error(`Token mint not found: ${pool.tokenMintA.toBase58()}`);
        }
        const tokenDecimalsB = (_b = (yield ctx.accountFetcher.getMintInfo(pool.tokenMintB, false))) === null || _b === void 0 ? void 0 : _b.decimals;
        if (!tokenDecimalsB) {
            throw new Error(`Token mint not found: ${pool.tokenMintB.toBase58()}`);
        }
        const tickArrayAddresses = getSurroundingTickArrayAddresses(pool, poolAddress, tickLower, tickUpper, ctx.program.programId);
        const tickArrays = yield ctx.accountFetcher.listTickArrays(tickArrayAddresses, refresh);
